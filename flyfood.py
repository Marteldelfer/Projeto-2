...
def abrir_mapa(mapa_arquivo:str): # retorna um dicionário com as coordenadas de cada ponto de interesse no mapa.
    # essas coordenadas são contadas começando do 1.
    matriz_mapa = {}
    ...
    with open(mapa_arquivo, 'r') as arquivo_de_entrada:
        ...
        for i in mapa:
            ...
            if i == ' ': # ignora os espaços
                continue

            elif len_x == 0: # para verificar os dois primeiros números
...
def get_distancia(a,b,matriz): # encontra a distancia entre 2 casas
    distancia_x = abs(matriz[a][0] - matriz[b][0])
    distancia_y = abs(matriz[a][1] - matriz[b][1])
...
for casa in caminho:
    print(casa, end=' ')
print()
print('distância percorrida:', tamanho)